...
正式 API 没有独立的曲风字段，因此 v5 只使用请求实际提供的歌曲名、艺人、专辑名和歌词；
...
| 指标 | 结果 |
| --- | ---: |
| 单标签 Top-1（最终本地输出） | 57.74% |
| 宏平均召回 | 52.57% |
| 单标签 Top-2 候选覆盖（候选池召回） | 74.88% |
| 单标签 Top-3 候选覆盖（候选池召回） | 81.86% |
| 单标签 Top-7 候选覆盖（候选池召回） | 95.01% |
| 单标签 Top-10 候选覆盖（候选池召回） | 97.84% |
| 全部样本 Top-3 候选覆盖（候选池召回） | 83.75% |
