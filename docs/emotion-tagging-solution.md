...
正式 v7 选择不加入旧快照的 54 首歌曲；该增强集保留为研究候选，未进入
接受的正式评测或服务包。

当前模型使用字符级 TF-IDF（`char_wb`、1–5 gram、最多 150,000 特征），
将歌曲名、艺人、专辑名重复 5 次后拼接歌词，再使用
...
正式 API 没有独立的曲风字段，因此 v7 只使用请求实际提供的歌曲名、艺人、专辑名和歌词；
...
| 指标 | 结果 |
| --- | ---: |
| 单标签 Top-1（最终本地输出） | 58.40% |
| 宏平均召回 | 53.19% |
| 单标签 Top-2 候选覆盖（候选池召回） | 75.04% |
| 单标签 Top-3 候选覆盖（候选池召回） | 82.20% |
| 单标签 Top-7 候选覆盖（候选池召回） | 94.84% |
| 单标签 Top-10 候选覆盖（候选池召回） | 97.67% |
| 全部样本 Top-3 候选覆盖（候选池召回） | 84.34% |
| 全部样本 Top-10 候选覆盖（候选池召回） | 98.61% |
