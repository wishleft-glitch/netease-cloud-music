...
正式 v8 选择不加入旧快照的 54 首歌曲；该增强集保留为研究候选，未进入
接受的正式评测或服务包。
...
正式 API 没有独立的曲风字段，因此 v8 只使用请求实际提供的歌曲名、艺人、专辑名、歌曲 ID 和歌词；
...
| 指标 | 结果 |
| --- | ---: |
| 单标签 Top-1（最终本地输出） | 58.90% |
| 宏平均召回 | 53.49% |
| 单标签 Top-2 候选覆盖（候选池召回） | 74.38% |
| 单标签 Top-3 候选覆盖（候选池召回） | 82.36% |
| 单标签 Top-7 候选覆盖（候选池召回） | 94.18% |
| 单标签 Top-10 候选覆盖（候选池召回） | 97.84% |
| 全部样本 Top-3 候选覆盖（候选池召回） | 84.24% |
| 全部样本 Top-10 候选覆盖（候选池召回） | 98.71% |
