...
训练时额外加入旧快照中独有的 54 首歌曲；与正式数据重叠的 5,043 个 ID
全部排除，避免把评测歌曲重复带入训练。
...
固定留出集的标签分布与训练集相近，去掉它能减少多数边界的过度补偿。SVC margin 在服务边界
转换为行 softmax，保证置信度落在 `[0,1]` 且排序与模型一致。
模型包还固化了一个低风险后处理：训练集中至少出现 2 首、且同一艺人标签一致度达到
80% 的艺人形成确定性标签映射；请求艺人命中映射时用于最终排序，映射随模型包一同版本化。

在固定留出集上：

| 指标 | 结果 |
| --- | ---: |
| 单标签 Top-1（最终本地输出） | 63.56% |
| 宏平均召回 | 58.27% |
| 单标签 Top-2 候选覆盖（候选池召回） | 76.04% |
| 单标签 Top-3 候选覆盖（候选池召回） | 85.52% |
| 单标签 Top-7 候选覆盖（候选池召回） | 95.51% |
| 单标签 Top-10 候选覆盖（候选池召回） | 97.17% |
| 全部样本 Top-3 候选覆盖（候选池召回） | 87.31% |
| 全部样本 Top-10 候选覆盖（候选池召回） | 98.22% |
...
2. 本地 TF-IDF/SVC 生成 Top-10 候选、置信度和 margin。
3. margin 足够大时直接放行，减少外部模型调用；歌词、标题、艺人和风格
   缺失时只使用真实存在的字段。
4. 低 margin（默认分差 < 0.10）样本送入内网语义复核器，提示词中只允许从 Top-10 候选中选一类，
...
Top-10 覆盖率只是语义复核阶段的候选池召回上限，绝不是 97% 的准确率。
